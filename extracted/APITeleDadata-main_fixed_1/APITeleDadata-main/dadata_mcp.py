...
async def fetch_company_via_mcp(inn: str) -> str:
    ...
    try:
        ...
        # OpenAI SDK здесь синхронный. Чтобы не блокировать event loop бота — уносим в отдельный поток.
        response = await asyncio.to_thread(
            ...
            input=(
                f"Проверь контрагента по ИНН {inn}. "
                "Надёжный ли он? Дай подробный анализ: реквизиты, статус, адрес, руководство, финансы."
            ),
        )
