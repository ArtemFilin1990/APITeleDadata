...
async def _safe_edit_or_send(msg: Message, text: str, reply_markup=None) -> None:
    """Пытаемся отредактировать служебное сообщение; если нельзя — отправляем новое."""
    try:
        await msg.edit_text(text, reply_markup=reply_markup)
    except Exception:
        logger.exception("Не удалось отредактировать сообщение — отправляю новое")
...
@router.message(CheckINN.waiting_inn, F.text)
@router.message(StateFilter(None), F.text.regexp(r"^[\d\s,;]+$"))
async def handle_inn(message: Message, state: FSMContext):
    ...
    try:
        for value in valid_values:
            # Дополнительный guard: даже если aiohttp зависнет на DNS/коннекте,
            # мы не оставляем пользователя на "Ищу данные..." навсегда.
            company = await asyncio.wait_for(fetch_company(value, raise_on_error=True), timeout=12)
            if company is None:
                not_found += 1
                continue
            found_companies.append((value, company))
    except Exception as exc:
        logger.exception("Ошибка при обработке ИНН/ОГРН")
        await _safe_edit_or_send(
            wait_msg,
            "⚠️ Не удалось получить данные из DaData. Проверь токен/лимиты/сеть и повтори запрос.",
