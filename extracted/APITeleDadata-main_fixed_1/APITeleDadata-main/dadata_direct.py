...
async def fetch_companies(
    ...
    *,
    ...
    Args:
        ...
        raise_on_error: если True — бросаем исключение на HTTP/сетевых ошибках,
            чтобы UI мог показать "не DaData не нашла", а "DaData не ответила / токен".
    """
    ...
    # - для головной организации (MAIN) — _PARTY_CACHE по простому ключу query (совместимо с fetch_company)
    # - прочие случаи — сохраняем в _BRANCHES_CACHE по cache_key
        ...
        async with _DADATA_SEM:
            ...
            async with session.post(DADATA_FIND_URL, json=payload, headers=headers) as resp:
                if resp.status != 200:
                    ...
                    if raise_on_error:
                        raise RuntimeError(f"DaData HTTP {resp.status}")
                    return []
    ...
    # Записываем в соответствующий кэш
    if branch_type == "BRANCH":
...
async def fetch_company(query: str, *, raise_on_error: bool = False) -> dict | None:
