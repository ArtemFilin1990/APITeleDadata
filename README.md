...
> Если токен скомпрометирован: в @BotFather выполните `/revoke`, получите новый токен и сразу обновите `TELEGRAM_BOT_TOKEN` (или `BOT_TOKEN`) в `.env`. Не храните реальные токены в репозитории, скриншотах и переписке.
...
- Учредители и список руководителей (если доступны в ответе DaData)
- Правопредшественники и правопреемники
- Система налогообложения и дата применения
- Лицензии и документы (агрегированные счётчики)
