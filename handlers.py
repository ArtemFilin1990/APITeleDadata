...
ERR_FETCH_TEXT = "⚠️ Не удалось получить данные из DaData. Попробуйте позже."
MAX_BATCH_SIZE = 10
...
def _build_result_totals(found: int, not_found: int, invalid: list[str]) -> str:
    ...
    if invalid:
        safe_invalid = ", ".join(html.escape(item) for item in invalid)
        lines.append(f"Невалидные значения: {safe_invalid}")
    return "\n".join(lines)
...
async def handle_inn(message: Message, state: FSMContext):
    ...
    values = list(dict.fromkeys(parse_inns(text)))
    if not values:
    ...
    if len(values) > MAX_BATCH_SIZE:
        await message.answer(
            f"За один запрос можно проверить не более {MAX_BATCH_SIZE} ИНН/ОГРН.",
    ...
    for value in valid_values:
        try:
            ...
            logger.exception("Ошибка обработки идентификатора %s", value)
            await wait_msg.edit_text(ERR_FETCH_TEXT, reply_markup=reply_main_menu_kb())
            return
    ...
    try:
        ...
        logger.exception("Не удалось обновить сообщение с карточкой компании")
        await message.answer(f"{_build_main_card(first_company)}\n\n{summary}", reply_markup=inline_actions_kb())
