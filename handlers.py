...
START_TEXT = (
    ...
    "Введите ИНН/ОГРН — соберу карточку и ключевые риски."
)
HELP_TEXT = (
    "Команды:\n"
    "/start — приветствие\n"
...
def _related_person_links(company: dict) -> list[str]:
    d = _d(company)
    founders = d.get("founders") if isinstance(d.get("founders"), list) else []
    managers = d.get("managers") if isinstance(d.get("managers"), list) else []
    ...
    for item in founders:
        if not isinstance(item, dict):
            continue
        person_type = str(item.get("type") or "").upper()
        person_inn = str(item.get("inn") or "").strip()
        if person_type in {"PHYSICAL", "INDIVIDUAL"} or len(person_inn) == 12:
            name = _v(item.get("name") or item.get("fio") or item.get("value"))
            if name != "—":
                links.append(f"- Учредитель: {name}")
    ...
    for item in managers:
        ...
        if person_type in {"PHYSICAL", "INDIVIDUAL"} or len(person_inn) == 12:
            ...
            if name == "—":
                continue
            links.append(f"- Руководитель: {name}" + (f" ({post})" if post else ""))
...
def _build_internal_scoring(company: dict) -> str:
    d = _d(company)
    ...
    if status and status != "ACTIVE":
        score -= 40
        factors.append(f"- Негативный статус в ЕГРЮЛ/ЕГРИП: {_v(state.get('status'))} (-40)")

    debt_value = debts.get("debt")
    try:
        debt_amount = float(str(debt_value).replace(" ", "").replace(",", ".")) if debt_value is not None else 0.0
    except ValueError:
        debt_amount = 0.0
    ...
    if debt_amount > 0:
        if debt_amount >= 1_000_000:
            score -= 30
            factors.append("- Крупная задолженность по ФНС (-30)")
        elif debt_amount >= 100_000:
            score -= 20
            factors.append("- Значимая задолженность по ФНС (-20)")
        else:
            score -= 10
            factors.append("- Наличие задолженности по ФНС (-10)")

    if not management.get("name"):
        score -= 10
        factors.append("- Нет данных о руководителе (-10)")

    licenses = d.get("licenses") if isinstance(d.get("licenses"), list) else []
    if licenses:
        score += 5
        factors.append("- Есть действующие лицензии (+5)")
    ...
    lines = [
        "📉 Внутренний скоринг",
        "(эвристический индикатор на основе доступных полей DaData)",
        f"Балл: {score}/100 • Уровень: {level}",
    ]
    ...
    if factors:
        ...
        lines.append("")
        lines.append("Факторы: недостаточно данных для детализации")
...
def _format_page(company: dict, page: str) -> str:
    ...
    if page == CB_PAGE_RELATIONS:
        links = _related_person_links(company)
        if not links:
            return "\n".join([
                "🔗 Связи с физлицами",
                "данные не предоставлены",
            ])
        return "\n".join(["🔗 Связи с физлицами", *links])

    if page == CB_PAGE_SCORING:
        return _build_internal_scoring(company)
